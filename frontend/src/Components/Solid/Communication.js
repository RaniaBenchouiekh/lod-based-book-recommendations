...
//export default class Communication extends Component {
    ...
    export async function createAppDataFile(uri, numberOfCnx, firstVisit) {
        await removeSolidFile(uri+'public/SolidGoogleBooks/appdata.ttl')
        ...
        graph.add(documentNN, SBA('performedConnexions'), numberOfCnx);
        graph.add(documentNN, SBA('firstVisit'), firstVisit);
    ...
    export async function readAppDataFile(uri){
        let dataUrl = uri + '/public/SolidGoogleBooks/appdata.ttl';

        let appData = await fileClient.readFile(dataUrl).then(data => {
            let graph = rdfLib.graph();
            rdfLib.parse(data, graph, dataUrl, "text/turtle");
            let blankNode = graph.any(undefined, SBA('performedConnexions')); 
                 
            let cnx = graph.any(blankNode, SBA('performedConnexions'))===null ? "none" : graph.any(blankNode, SBA('performedConnexions')).value
            let visit = graph.any(blankNode, SBA('firstVisit'))===null ? "none" : graph.any(blankNode, SBA('firstVisit')).value
            let str = cnx + "%%" + visit 
    ...
    export async function createInfoPersoFile(uri, fn, ln, email, birthday, gender) {
        await removeSolidFile(uri+'public/SolidGoogleBooks/infoperso.ttl')
    ...
    export async function createSavedBooksFile(uri, ISBN, title, subtitle, abstract, numberOfPages, authors, language, releaseDate, literaryGenre, urlPhoto, provider, link) {
        ...
        //if(await checkSavedBook(uri, title)) {
            await removeSavedBook(uri, title + ".ttl")
        //}
    ...
    export async function createRatedBooksFile(uri, ratingValue, ISBN, title, subtitle, abstract, numberOfPages, authors, language, releaseDate, publisher, literaryGenre, urlPhoto, provider, link) {
        ...
        //if(await checkRatedBook(uri, title)) {
            await removeRatedBook(uri, title + ".ttl")
        //}
